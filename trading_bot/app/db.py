...
class PostgresRuntimeConnection:
    ...
    def _normalize_sql(self, sql):
        if not isinstance(sql, str):
            return sql
        sql = re.sub(r"\?", "%s", sql)
        sql = self._rewrite_sqlite_ddl(sql)
        sql = self._rewrite_bool_comparisons(sql)
        sql = self._rewrite_bool_insert_values(sql)
        sql = re.sub(r"last_insert_rowid\s*\(\s*\)", "lastval()", sql, flags=re.IGNORECASE)
        return sql

    @staticmethod
    def _rewrite_sqlite_ddl(sql: str) -> str:
        sql = re.sub(
    ...
    def _rewrite_bool_comparisons(self, sql: str) -> str:
        return self._BOOLEAN_TOKEN_RE.sub(self._rewrite_bool_literal, sql)
    ...
    def _rewrite_bool_insert_values(self, sql: str) -> str:
        pattern = re.compile(
            r"INSERT\s+INTO\s+[^\n(]+\s*\((?P<cols>.*?)\)\s*VALUES\s*\((?P<vals>.*?)\)\s*(?=ON\s+CONFLICT|$)",
        ...
        cols = self._split_top_level(match.group("cols"))
        values = self._split_top_level(match.group("vals"))
        if not cols or len(cols) != len(values):
            return sql

        rewritten_values = list(values)
        for idx, column_name in enumerate(cols):
            normalized = self._normalize_identifier(column_name)
            if normalized in self._BOOLEAN_COLUMN_NAMES and idx < len(rewritten_values):
                value = rewritten_values[idx].strip()
                if value in {"0", "1"}:
                    rewritten_values[idx] = "TRUE" if value == "1" else "FALSE"

        start = match.start("vals")
        end = match.end("vals")
        prefix = sql[:start]
        suffix = sql[end:]
        return f"{prefix}{', '.join(rewritten_values)}{suffix}"
    ...
    def _split_top_level(raw: str) -> list[str]:
        ...
        for ch in raw:
            if quote:
                token.append(ch)
                if ch == quote and (len(token) < 2 or token[-2] != "\\"):
                    quote = None
                continue
            if ch in ("'", '"'):
                quote = ch
    ...
    def _normalize_params(self, sql: str, params):
        ...
        bool_positions = self._boolean_param_positions(sql, len(converted))
        for index in bool_positions:
            if 0 <= index < len(converted) and isinstance(converted[index], int) and converted[index] in (0, 1):
                converted[index] = bool(converted[index])

        table_name = self._extract_target_table(sql)
        if table_name and self._normalize_identifier(table_name) == "brokers":
            timestamp_positions = self._timestamp_param_positions(sql, len(converted))
            for index in timestamp_positions:
                if 0 <= index < len(converted):
                    value = converted[index]
                    if isinstance(value, (int, float)):
                        converted[index] = datetime.fromtimestamp(float(value), tz=timezone.utc)
        return tuple(converted)

    @staticmethod
    def _extract_target_table(sql: str) -> str | None:
        match = re.search(r"INSERT\s+INTO\s+([A-Za-z0-9_\.\"`\[]+)\s*\(", sql, flags=re.IGNORECASE)
        if match:
            return match.group(1)
        match = re.search(r"UPDATE\s+([A-Za-z0-9_\.\"`\[]+)\s+SET", sql, flags=re.IGNORECASE)
        if match:
            return match.group(1)
        return None

    def _timestamp_param_positions(self, sql: str, param_count: int) -> set[int]:
        positions: set[int] = set()
        insert_match = re.search(r"INSERT\s+INTO\s+[A-Za-z0-9_\.\"`\[]+\s*\((?P<cols>.*?)\)\s*VALUES\s*\((?P<vals>.*?)\)\s*(?=ON\s+CONFLICT|$)", sql, flags=re.IGNORECASE | re.DOTALL)
        if insert_match:
            cols = self._split_csv(insert_match.group("cols"))
            values = self._split_top_level(insert_match.group("vals"))
            if values:
                param_index = 0
                for value_index, value in enumerate(values):
                    if param_index >= param_count:
                        break
                    if value.strip() in {"?", "%s"}:
                        if value_index < len(cols) and self._normalize_identifier(cols[value_index]) in {"created_at", "updated_at"}:
                            positions.add(param_index)
                        param_index += 1
                return positions
            for index, col in enumerate(cols):
                if index >= param_count:
                    break
                if self._normalize_identifier(col) in {"created_at", "updated_at"}:
                    positions.add(index)
            return positions
        update_match = re.search(r"UPDATE\s+[A-Za-z0-9_\.\"`\[]+\s+SET\s+(?P<set>.*?)(?:\s+WHERE\b|\s*$)", sql, flags=re.IGNORECASE | re.DOTALL)
        if update_match:
            assignments = self._split_csv(update_match.group("set"))
            for index, assignment in enumerate(assignments):
                match = re.match(r"(?P<col>[A-Za-z0-9_\.\"`\[]+)\s*=\s*(?:\?|%s)", assignment, flags=re.IGNORECASE)
                if match and self._normalize_identifier(match.group("col")) in {"created_at", "updated_at"}:
                    positions.add(index)
        return positions

    def _boolean_param_positions(self, sql: str, param_count: int) -> set[int]:
        positions: set[int] = set()
        ...
        if insert_match:
            ...
            if values:
                ...
                for value_index, value in enumerate(values):
                    if param_index >= param_count:
                        break
                    if value.strip() in {"?", "%s"}:
                        if value_index < len(cols) and self._normalize_identifier(cols[value_index]) in self._BOOLEAN_COLUMN_NAMES:
                            positions.add(param_index)
                        param_index += 1
                return positions
            ...
            for index, col in enumerate(cols):
                if index >= param_count:
                    break
                if self._normalize_identifier(col) in self._BOOLEAN_COLUMN_NAMES:
                    positions.add(index)
            return positions
        ...
        if update_match:
            assignments = self._split_csv(update_match.group("set"))
            for index, assignment in enumerate(assignments):
                match = re.match(r"(?P<col>[A-Za-z0-9_\.\"]+)\s*=\s*(?:\?|%s|TRUE|FALSE|NULL)\b", assignment, flags=re.IGNORECASE)
                if match and self._normalize_identifier(match.group("col")) in self._BOOLEAN_COLUMN_NAMES:
                    positions.add(index)
            return positions

        for match in re.finditer(r"(?P<col>[A-Za-z0-9_\.\"]+)\s*=\s*(?P<value>\?|%s|TRUE|FALSE)", sql, flags=re.IGNORECASE):
            column = self._normalize_identifier(match.group("col"))
            if column in self._BOOLEAN_COLUMN_NAMES:
                placeholder_offset = sql[:match.start()].count("?") + sql[:match.start()].count("%s")
                positions.add(placeholder_offset)

        return positions

    @staticmethod
    def _split_csv(raw_value: str) -> list[str]:
        return [item.strip().strip('"`[]') for item in re.split(r",\s*(?![^(]*\))", raw_value) if item.strip()]
    ...
    with get_db() as conn:
        conn.execute(
            ...
            CREATE TABLE IF NOT EXISTS account_state (
                ...
                keep_terminal_alive INTEGER DEFAULT 0,
        ...
        _add_column_if_missing(conn, "account_state", "keep_terminal_alive", "INTEGER DEFAULT 0")
    ...
    with get_db() as conn:
        conn.execute(
            ...
            (
                ...
                int(bool(state.get("keep_terminal_alive", False))),
    ...
    with get_db() as conn:
        if current["is_default"]:
            ...
            if row:
                ...
                conn.execute(
                    "UPDATE brokers SET is_default = 0 WHERE id != ?",
                    (broker_id,),
                )
                conn.execute(
                    "UPDATE brokers SET is_default = 1, is_active = 1, updated_at = ? WHERE id = ?",
                    (int(time.time()), replacement_id),
                )
        conn.execute("DELETE FROM brokers WHERE id = ?", (broker_id,))

    state = get_account_state()
    if state.get("data_feed_broker_id") == broker_id:
        next_broker = get_default_broker() if replacement_id is None else get_broker(replacement_id)
        state["data_feed_broker_id"] = next_broker["id"] if next_broker else None
        save_account_state(state)

    if current["is_default"] and replacement_id is not None:
        default_now = get_default_broker()
        if default_now and default_now["id"] != replacement_id:
            set_default_broker(replacement_id)
    elif current["is_default"] and replacement_id is None:
        state = get_account_state()
        state["data_feed_broker_id"] = None
        save_account_state(state)
