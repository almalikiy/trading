...
export default function App({ darkMode, setDarkMode }) {
  ...
  const refreshAccountState = useCallback(() => {
    fetchJson(`${getBackendUrl("mt5", "http")}/account/state`)
      .then((data) => {
  ...
  const refreshBrokers = useCallback(() => {
    fetchJson(`${getBackendUrl("mt5", "http")}/brokers?include_inactive=true`)
      .then((data) => {
    ...
    fetchJson(`${getBackendUrl("mt5", "http")}/brokers/default`)
      .then((data) => {
        if (data && data.id) {
  ...
  const refreshAutoTradeHealth = () => {
    fetchJson(`${getBackendUrl("mt5", "http")}/account/auto_trade_health`)
  ...
  useEffect(() => {
    ...
    postJson(`${getBackendUrl("mt5", "http")}/account/set_auto_trade_enabled`, autoTradeEnabled).catch(() => { });
  }, [autoTradeEnabled, accountSettingsLoaded]);

  useEffect(() => {
    if (!accountSettingsLoaded) return;
    postJson(`${getBackendUrl("mt5", "http")}/account/set_keep_terminal_alive`, keepTerminalAlive).catch(() => { });
  ...
  useEffect(() => {
    ...
    const fetchSignal = () => {
      if (inFlight) return;
      inFlight = true;
      fetchJson(`${baseUrl}/signal?symbol=${symbol}&mode=${tradeMode}`)
        .then((data) => {
          if (data.error) {
  ...
  useEffect(() => {
    ...
    const fetchOhlcv = () => {
      if (inFlight) return;
      inFlight = true;
      fetchJson(`${baseUrl}/ohlcv?symbol=${symbol}&timeframe=${tf}&bars=${barCount}`)
        .then((data) => {
  ...
  useEffect(() => {
    ...
    const fetchBrokerOrderStatus = () => {
      if (inFlight) return;
      inFlight = true;
      fetchJson(`${getBackendUrl("mt5", "http")}/brokers/${brokerId}/order_status?symbol=${symbol}`)
  ...
  const handleManualOpen = async (tradeType) => {
    ...
    try {
      const data = await postJson(`${getBackendUrl("mt5", "http")}/trade/open_v2`, {
        symbol,
        lot,
  ...
  const handleSaveTradeTPSL = async (tradeId) => {
    ...
    try {
      const data = await postJson(`${getBackendUrl("mt5", "http")}/trade/update_tpsl`, {
        trade_id: id,
  ...
  const handleCloseTrade = async (trade, index) => {
    ...
    try {
      ...
      if (String(trade.execution_mode || "").toLowerCase() === "mouse") {
        data = await postJson(`${getBackendUrl("mt5", "http")}/trade/close_by_index`, { index });
      } else {
        data = await postJson(`${getBackendUrl("mt5", "http")}/trade/close`, {
          symbol: trade.symbol,
          lot: trade.lot,
          ticket: trade.ticket,
        });
  ...
  return (
    <Box sx={{
      p: { xs: 1, sm: 2 }, maxWidth: 1200, mx: "auto", width: "100%",
        ...
        <Grid container spacing={2} alignItems="center">
          ...
          <Grid item xs={12} sm={3}>
            <FormControl size="small" fullWidth>
              <InputLabel id="bar-count-label">Power Range</InputLabel>
      ...
      <Paper sx={{ p: { xs: 1, sm: 2 }, mb: 2, overflowX: "auto", width: "100%" }}>
        <Box sx={{ display: "flex", flexDirection: { xs: "column", md: "row" }, gap: 2 }}>
          {/* Panel Signal */}
          <Box sx={{
            flex: 1, minWidth: 0, borderRight: { md: "1px solid #eee" },
          ...
          {/* Panel Trading */}
          <Box sx={{
            flex: 1, minWidth: 0, borderLeft: { md: "1px solid #eee" }, pl: { md: 2, xs: 0 }, mt: { xs: 2, md: 0 },
            borderRadius: 1,
            border: `2px solid ${signal === "wait" ? "#bdbdbd" : getSignalColor(signal)}`,
            animation: signal === "wait" ? "none" : "tradePanelPulse 1s infinite",
            "@keyframes tradePanelPulse": {
